...
processFiles <- function(x) {
  ...
  dat <- fread(x, select = c("aminoAcid", "vGeneName", "jGeneName", "sequenceStatus"))
...
naive <-
  data.table(paste(naive$aminoAcid, naive$vGeneName, naive$jGeneName, sep = " "))
...
vaccs <-
  data.table(paste(vaccs$aminoAcid, vaccs$vGeneName, vaccs$jGeneName, sep = " "))
...
for (i in 1:numpre)
  navcounts[i] <-
  print(sum(unlist(lapply(finally$names, function(x)
    length(grep(x, greplistpre[i]))))))
...
navvac_hist <- ggplot(melt(cbind(navpercs, vacpercs)), aes(x = value, fill = Var2)) +
  geom_histogram(binwidth=0.01, color=1) +
  labs(title = "Percentage of Clonotypes", 
       y = "Frequency",
       x = "Percentages",
       fill = element_blank()) +
...
ggsave(filename = "HistClonotypes.pdf",
       plot = navvac_hist,
       width = 5,
       height = 5,
       units = "in",
