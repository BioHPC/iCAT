                  ...
                  tabPanel(title = "Training",
                           ...
                           sidebarLayout(
                             sidebarPanel(
                               ...
                               hidden(textInput("otherbt", "Please provide data columns to analyze by (space-separated)",
...
server <- function(input, output, session) {
  ...
  preds <- eventReactive(input$pred, {
    show("h3")
    show("dnPred")
    return(pred(both(), readPost(input$indpt$datapath, field), input$indpt$datapath, input$indpt$name, field))
  })
